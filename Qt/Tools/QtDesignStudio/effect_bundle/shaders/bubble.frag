...
void POST_PROCESS()
{
	...
	vec3 fColor = (sceneSample_refr + sceneSample_refl/2.0);// * vColor.rgb;
	float a = vColor.a*max(0.25,fresnel);//min(max(0.0,centerDist*smoothstep(0.0,1.0,vColor.a)),1.0);
	a *= opacity;
	vec3 lighting = vec3(DIFFUSE.rgb + SPECULAR.rgb + EMISSIVE.rgb);
    COLOR_SUM = vec4((fColor+lighting)*baseColor.rgb,a);
}