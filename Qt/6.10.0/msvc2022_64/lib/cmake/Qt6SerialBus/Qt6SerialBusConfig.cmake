...
set(__qt_SerialBus_always_load_private_module OFF)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6SerialBus-build.cmake" OPTIONAL)
if (Qt6SerialBus_FOUND
        AND NOT Qt6SerialBusPrivate_FOUND
        AND NOT FALSE
        AND (
            __qt_SerialBus_always_load_private_module
    ...
    find_package(Qt6SerialBusPrivate "6.10.0" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6SerialBusPrivate_FOUND)
        get_property(Qt6SerialBusPrivate_warning_shown GLOBAL PROPERTY
            Qt6SerialBusPrivate_warning_shown
        )
        if(NOT Qt6SerialBusPrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6SerialBusPrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6SerialBusPrivate_warning_shown ON
            )
        endif()
    endif()
endif()
unset(__qt_SerialBus_always_load_private_module)
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6SerialBus_FOUND)
    ...
    if(NOT Qt6SerialBus_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6SerialBus_DEFINITIONS)
        ...
        endforeach()
        set(Qt6SerialBus_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6SerialBus_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6SerialBus${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6SerialBus${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6SerialBus_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6SerialBus${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6SerialBus${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::SerialBus)
    ...
    if(NOT "SerialBus" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6SerialBusVersionlessAliasTargets.cmake")
        endif()
    endif()
else()
