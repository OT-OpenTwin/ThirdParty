...
set(__qt_RepParser_always_load_private_module OFF)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6RepParser-build.cmake" OPTIONAL)
if (Qt6RepParser_FOUND
        AND NOT Qt6RepParserPrivate_FOUND
        AND NOT TRUE
        AND (
            __qt_RepParser_always_load_private_module
    ...
    find_package(Qt6RepParserPrivate "6.10.0" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6RepParserPrivate_FOUND)
        get_property(Qt6RepParserPrivate_warning_shown GLOBAL PROPERTY
            Qt6RepParserPrivate_warning_shown
        )
        if(NOT Qt6RepParserPrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6RepParserPrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6RepParserPrivate_warning_shown ON
            )
        endif()
    endif()
endif()
unset(__qt_RepParser_always_load_private_module)
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6RepParser_FOUND)
    ...
    if(NOT Qt6RepParser_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6RepParser_DEFINITIONS)
        ...
        endforeach()
        set(Qt6RepParser_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6RepParser_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6RepParser${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6RepParser${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6RepParser_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6RepParser${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6RepParser${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::RepParser)
    ...
    if(NOT "RepParser" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6RepParserVersionlessAliasTargets.cmake")
        endif()
    endif()
else()
