...
if (NOT QT_NO_CREATE_TARGETS AND Qt6Linguist_FOUND)
    ...
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6LinguistAdditionalTargetInfo.cmake"
        OPTIONAL)
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6LinguistExtraProperties.cmake"
        OPTIONAL)
endif()

# Find the private module counterpart.
set(__qt_Linguist_always_load_private_module OFF)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6Linguist-build.cmake" OPTIONAL)
if (Qt6Linguist_FOUND
        AND NOT Qt6LinguistPrivate_FOUND
        AND NOT TRUE
        AND (
            __qt_Linguist_always_load_private_module
    ...
    find_package(Qt6LinguistPrivate "6.10.0" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6LinguistPrivate_FOUND)
        get_property(Qt6LinguistPrivate_warning_shown GLOBAL PROPERTY
            Qt6LinguistPrivate_warning_shown
        )
        if(NOT Qt6LinguistPrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6LinguistPrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6LinguistPrivate_warning_shown ON
            )
        endif()
    endif()
endif()
unset(__qt_Linguist_always_load_private_module)
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6Linguist_FOUND)
    ...
    if(NOT Qt6Linguist_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6Linguist_DEFINITIONS)
        ...
        endforeach()
        set(Qt6Linguist_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6Linguist_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6Linguist${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6Linguist${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6Linguist_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6Linguist${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6Linguist${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::Linguist)
    ...
    if(NOT "Linguist" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6LinguistVersionlessAliasTargets.cmake")
        endif()
    endif()
else()
