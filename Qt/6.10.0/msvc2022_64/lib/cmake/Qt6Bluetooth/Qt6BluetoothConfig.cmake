...
set(__qt_Bluetooth_always_load_private_module OFF)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6Bluetooth-build.cmake" OPTIONAL)
if (Qt6Bluetooth_FOUND
        AND NOT Qt6BluetoothPrivate_FOUND
        AND NOT FALSE
        AND (
            __qt_Bluetooth_always_load_private_module
    ...
    find_package(Qt6BluetoothPrivate "6.10.0" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6BluetoothPrivate_FOUND)
        get_property(Qt6BluetoothPrivate_warning_shown GLOBAL PROPERTY
            Qt6BluetoothPrivate_warning_shown
        )
        if(NOT Qt6BluetoothPrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6BluetoothPrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6BluetoothPrivate_warning_shown ON
            )
        endif()
    endif()
endif()
unset(__qt_Bluetooth_always_load_private_module)
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6Bluetooth_FOUND)
    ...
    if(NOT Qt6Bluetooth_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6Bluetooth_DEFINITIONS)
        ...
        endforeach()
        set(Qt6Bluetooth_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6Bluetooth_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6Bluetooth${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6Bluetooth${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6Bluetooth_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6Bluetooth${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6Bluetooth${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::Bluetooth)
    ...
    if(NOT "Bluetooth" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6BluetoothVersionlessAliasTargets.cmake")
        endif()
    endif()
else()
