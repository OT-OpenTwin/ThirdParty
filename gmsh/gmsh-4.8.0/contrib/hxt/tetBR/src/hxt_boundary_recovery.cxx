...
int tetgenmesh::reconstructmesh(void *p){
  ...
  
  {
    ...
    for (long long i = 0; i < mesh->tetrahedra.num; i++) {
      triface tf1 = ts[i];
      ...
      while (pointloop != (point) NULL) {
        if (issteinerpoint(pointloop)) {
          ...
          if (pointtype(pointloop) == FREESEGVERTEX) {
            return HXT_ERROR_MSG(HXT_STATUS_ERROR, "Steiner point inserted that splits an existing edge"
                        "(boundary mesh has changed,"
                        "consider refining the mesh or switching to the delauney mesher)");
