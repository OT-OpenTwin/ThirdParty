...
IFloatingWidget* DockAreaTitleBarPrivate::makeAreaFloating(const QPoint& Offset, eDragState DragState)
{
	...
	if (CreateFloatingDockContainer)
	{
        ...
        FloatingWidget = FloatingDockContainer = CComponentsFactory::factory()
                ->createFloatingDockContainer(DockArea);
	}
	else
	{
        auto w = CComponentsFactory::factory()->createFloatingDragPreview(DockArea);
...
void DockAreaTitleBarPrivate::waylandPreviewMove(QMouseEvent* ev)
{
	...
	auto FloatingDockContainer = CComponentsFactory::factory()->createFloatingDockContainer(DockArea);
	auto TopLevelDockWidget = FloatingDockContainer->topLevelDockWidget();
	if (TopLevelDockWidget)
	{
		TopLevelDockWidget->emitTopLevelChanged(true);
