...
void FloatingDragPreviewPrivate::createFloatingWidget()
{
	...
	{
		FloatingWidget = CComponentsFactory::factory()->createFloatingDockContainer(DockWidget);
	}
	else if (DockArea && DockArea->features().testFlag(CDockWidget::DockWidgetFloatable))
	{
		FloatingWidget = CComponentsFactory::factory()->createFloatingDockContainer(DockArea);
	}
